...
import 'package:gymboo_admin/utils/loaders/animation_loader.dart';

/// Custom PaginatedDataTable widget with additional features
class gbPaginatedDataTable extends StatelessWidget {
  const gbPaginatedDataTable({
    super.key,
    ...
    this.dataRowHeight = gbSizes.xl * 2,
    ...
    return SizedBox(
      ...
      child: Theme(
        ...
        child: PaginatedDataTable2(
          ...
          headingRowColor: WidgetStateProperty.resolveWith((states) => gbColors.primaryBackground),
          empty: gbAnimationLoaderWidget(animation: gbImages.packageAnimation, text: 'Nothing Found', height: 200, width: 200),
          headingRowDecoration: const BoxDecoration(
            borderRadius: BorderRadius.only(
              topLeft: Radius.circular(gbSizes.borderRadiusMd),
              topRight: Radius.circular(gbSizes.borderRadiusMd),
            ),
          ),
          sortArrowBuilder: (bool ascending, bool sorted) {
            if (sorted) {
              return Icon(ascending ? Iconsax.arrow_up_3 : Iconsax.arrow_down, size: gbSizes.iconSm);
            } else {
              return const Icon(Iconsax.arrow_3, size: gbSizes.iconSm);
            }
          },
        ),
      ),
