...
class gbTableActionButtons extends StatelessWidget {
  const gbTableActionButtons({
    super.key,
    this.view = false,
    this.edit = true,
    this.delete = true,
      ...
      children: [
        if (view)
          IconButton(
            ...
            icon: const Icon(Iconsax.eye, color: gbColors.darkerGrey),
          ),
        if (edit)
          IconButton(
            onPressed: onEditPressed,
            icon: const Icon(Iconsax.pen_add, color: gbColors.primary),
          ),
        if (delete)
          IconButton(
            onPressed: onDeletePressed,
            icon: const Icon(Iconsax.trash, color: gbColors.error),
          ),
      ],
    );
  }
